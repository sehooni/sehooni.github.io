...
최근 AI 학계와 현업에서 가장 널리 사용되는 LLM(대형 언어 모델) 연동 패턴은 단연 **RAG(Retrieval-Augmented Generation, 검색 증강 생성)**입니다. 하지만 현재 대다수의 RAG 시스템은 문서를 단순 파싱해 벡터 데이터베이스에 올려두고, 사용자가 질문할 때마다 필요한 조각을 임시로 찾아와 대답하는 수준에 머물러 있습니다. 이는 정보가 유기적으로 연결되거나 시간이 지남에 따라 스스로 축적·진화하는 구조와는 거리가 멉니다.

이러한 상황에서, 전 OpenAI 공동 창립자이자 Tesla AI 디렉터였던 **안드레이 카파시(Andrej Karpathy)**가 제안한 **'LLM Wiki'** 패턴은 LLM 기반의 개인 지식 관리 시스템(PKM, Personal Knowledge Management)에 새로운 이정표를 제시합니다. 

카파시는 최근 자신의 Gist를 통해 **"Obsidian은 IDE이고, LLM은 개발자이며, Wiki는 소스 코드이다(Obsidian is the IDE; the LLM is the programmer; the wiki is the codebase)"**라는 매력적인 비유와 함께, LLM이 스스로 편집하고 조율하는 정적 마크다운 지식 컴파일 레이어의 설계 방법론을 공개했습니다. 
...
> 💡 **LLM Wiki의 핵심 통찰**: LLM은 지루해하지 않고, 상호 참조 링크 추가를 까먹지 않으며, 한 번의 처리(Pass)로 10~15개의 관련 마크다운 파일을 정교하게 읽고 쓸 수 있습니다. 따라서 **"인간이 수집하고 기획하며(Curate), 갱신과 조율 등의 단순 노동은 LLM에게 맡기는 위키"**를 구축하자는 것이 LLM Wiki의 핵심입니다.
...
- 이 레이어는 **불변(Immutable)**입니다. LLM은 이 문서들을 읽어서 지식을 축적할 뿐, 원본 파일을 임의로 수정하거나 삭제하지 않습니다. 위키 내 모든 합성 정보의 신뢰성을 담보하는 '최종 진실의 원천(Source of Truth)' 역할을 합니다.
...
- 이 레이어는 지속적으로 **컴파일 및 병합(Compiled & Compounded)**되는 지식의 실체입니다.
...
- **피드백 결합**: 만약 사용자가 "A 기법과 B 기법의 장단점을 표로 정리해 줘"라고 요청하고 LLM이 내놓은 비교 결과가 훌륭하다면, 이 답변은 일회성 채팅으로 끝나는 것이 아니라 **"A_vs_B_Comparison.md"와 같은 신규 위키 페이지로 자동 컴파일되어 저장**됩니다. 사용자의 모든 탐구 활동의 흔적이 고스란히 위키의 자산으로 영구 누적됩니다.
...
과거에는 LLM이 더 똑똑하게 답변하도록 유도하기 위해 프롬프트를 다듬는 '프롬프트 엔지니어링(Prompt Engineering)'이 주를 이루었습니다. 하지만 모델들의 콘텍스트 윈도우 크기와 검색 효율성이 증가함에 따라, 이제 초점은 **'메모리 아키텍처(Memory Architecture)'**로 이동하고 있습니다.

안드레이 카파시가 제안한 **LLM Wiki**는 LLM을 단순한 '일회용 대화 상대'가 아니라, **시간에 따라 스스로 정리되고 공고화되는 지식의 집적 체계를 코딩하는 주체**로 재설정합니다. RAG의 한계인 무누적성과 반복 비용을 극복하고, 인간의 위키 관리 오버헤드를 제로에 수렴하게 함으로써 진정으로 나와 호흡하는 개인용 비서의 든든한 뇌 세포망을 손쉽게 구축할 수 있게 되었습니다.

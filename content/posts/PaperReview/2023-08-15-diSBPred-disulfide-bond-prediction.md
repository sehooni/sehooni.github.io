...
![Table 1: Recent works on the prediction of disulfide bonding state.](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/table1.jpg)
*Table 1: 이황화 결합 상태 예측 분야의 최근 관련 연구 및 방법론 요약*
...
이황화 결합 예측을 단일 단계에서 직접 수행하지 않고 2단계로 분할하는 이유는 **생물학적 현실과** **계산적 필요성** 모두에서 비롯됩니다.
...
- 이후, 반응 가능한 두 시스테인이 3차원 공간상에서 **물리적으로 충분히 가까이 위치하여야** S-S 결합이 형성됩니다.

이 생물학적 2단계 과정을 그대로 반영한 것이 diSBPred의 설계입니다.

**계산적 관점에서**, 단백질 서열 내에 $N$개의 시스테인 잔기가 존재할 때 단순 조합으로 결합 쌍을 탐색하면 $\binom{N}{2} = \frac{N(N-1)}{2}$의 후보 쌍을 전부 평가해야 합니다. 예컨대 20개의 시스테인이 존재하는 단백질에서는 190개의 후보 쌍이 생성되지만, 실제 결합 쌍은 일반적으로 5-10개에 불과합니다. 이로 인해 **양성:음성 비율이 1:20 이상의** 극심한 데이터 불균형(Class Imbalance)이 발생하여, 분류기가 모든 입력을 "비결합"으로 예측하더라도 높은 정확도를 달성하는 허위 학습(Trivial Solution) 문제에 빠지게 됩니다.
...
이 2단계 설계의 가장 큰 이점은 **1단계에서 후보를 사전 필터링하여** 2단계의 탐색 공간을 극적으로 압축($O(N^2) \rightarrow O(M^2)$, $M \ll N$)하는 것입니다. 동시에, 1단계의 출력 확률이 2단계의 입력 피처로 활용되므로 두 단계 간의 **정보 전달(Information Flow)이** 자연스럽게 이루어집니다.

![diSBPred Prediction Framework](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/fig3.jpg)
*Figure 3: diSBPred의 2단계 계층적 예측 프레임워크 및 Stacking 앙상블 아키텍처 상세*
...
Base Classifier들의 예측 결과를 **새로운 피처로 재구성하여** 최종 분류기를 학습합니다:

$$\mathbf{z}_i = [P_{\text{ET}}(C_i), P_{\text{LGB}}(C_i), P_{\text{LR}}(C_i), P_{\text{KNN}}(C_i)]$$

이 메타 피처 벡터 $\mathbf{z}_i$를 입력으로 받는 **로지스틱 회귀(Logistic Regression, LR)** 메타 분류기가 최종 분류 결정을 내립니다. 중요한 점은, Base Classifier의 학습과 메타 피처 생성 시 **K-Fold 교차 검증(Cross-Validation)을** 통해 각 폴드의 Out-of-Fold 예측값만을 메타 피처로 사용한다는 것입니다. 이를 통해 메타 분류기가 Base Classifier의 훈련 데이터에 대한 과적합된 예측값을 학습하는 것을 방지합니다.
...
![diSBPred Overall Pipeline](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/fig1.jpg)
...
**서열 중복 제거(Redundancy Reduction):** CD-HIT 알고리즘을 활용하여 서열 유사도(Sequence Identity)가 **25% 이상인** 중복 서열을 제거합니다. 이 엄격한 기준(25%)은 구조적 유사성이 있을 수 있는 원격 상동(Remote Homology) 서열까지 배제하여, 모델이 서열 유사성에 의존한 암기(Memorization)가 아닌 일반적인 물리화학적 패턴을 학습하도록 강제합니다.
...
![diSBPred Sliding Window & Feature Mining](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/fig2.jpg)
*Figure 2: 다양한 피처 추출 도구를 이용한 아미노산 서열의 다차원 피처 벡터 인코딩 과정*
...
![diSBPred Window Size Selection for Stage 1](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/fig5.jpg)
*Figure 5: 개별 시스테인 결합 상태 예측(Stage 1)에서 슬라이딩 윈도우 크기에 따른 LightGBM 모델의 Balanced Accuracy 비교*

![diSBPred Window Size Selection for Stage 2](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/fig6.jpg)
*Figure 6: 이황화 결합 연결성 예측(Stage 2)에서 슬라이딩 윈도우 크기에 따른 LightGBM 모델의 Balanced Accuracy 비교*

![Table 6: Performance of cysteine bonding state prediction model, trained with the best window size.](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/table6.jpg)
*Table 6: 최적의 윈도우 크기(W=17)로 학습된 개별 시스테인 결합 상태 예측 모델의 세부 성능 지표*

서열의 양 끝단에서 윈도우가 범위를 초과하는 경우에는 **제로 패딩(Zero Padding)을** 적용하여 모든 시스테인에 대해 동일한 차원의 피처 벡터를 보장합니다.

추출되는 피처 종류는 총 **6가지 범주로** 구성되며, 각각이 이황화 결합 형성의 서로 다른 물리화학적 측면을 인코딩합니다:

#### 3.2.1. Residue Profile (서열 프로파일)

가장 기본적인 서열 정보를 인코딩합니다:
- 윈도우 내의 각 아미노산을 **20차원 One-hot 인코딩으로** 변환합니다. 17개 잔기 × 20차원 = **340개의 피처가** 생성됩니다.
...
각 물리화학적 속성은 윈도우 내 17개 잔기에 대해 연속 수치값으로 표현되므로, 17 × 5 = **85개의 피처가** 추가됩니다.

#### 3.2.3. Conservation Profile (진화 보존 프로파일 - PSSM)

진화적으로 보존된 시스테인은 기능적으로 중요한 이황화 결합에 참여할 확률이 높습니다. 이를 포착하기 위해 **PSI-BLAST를** NR(Non-Redundant) 데이터베이스에 대해 3회 반복 실행하여 PSSM(Position-Specific Scoring Matrix)을 구축합니다:

- **1D Monogram (MG):** PSSM의 각 위치에서 20개 표준 아미노산이 나타날 진화적 보존 강도(Log-odds score)를 직접 추출합니다. 이는 "이 위치에서 이 아미노산이 보존되어야 하는 진화적 압력"을 수치화한 것으로, 시스테인 위치의 MG 점수가 높을수록 해당 시스테인이 기능적으로 보존되어 이황화 결합에 참여할 확률이 높습니다. 17 × 20 = **340개의 피처를** 생성합니다.

- **2D Bi-gram (BG):** 서열 상에서 **인접한 두 아미노산 쌍의** 공동 보존 패턴을 반영합니다. PSSM에서 연속된 두 위치의 점수를 결합하여 $20 \times 20 = 400$차원의 전이 행렬(Transition Matrix)을 구성합니다. 이를 통해 "시스테인 앞에 특정 아미노산이 보존적으로 나타나는 패턴"과 같은 2차 진화 관계를 인코딩합니다.
...
- **용매 접근성(Accessible Surface Area, ASA):** 각 잔기의 용매 노출 정도를 수치화합니다. 이 피처가 중요한 이유는 생물학적으로 명확합니다: **용매에 노출된 시스테인은** 환원제(예: 글루타티온)와의 반응이 용이하여 **자유 상태(Free)로** 존재할 확률이 높고, **단백질 중심부에 매몰된 시스테인은** 산화 환경에서 보호되어 **이황화 결합을** 형성·유지할 확률이 높습니다.
...
![diSBPred Feature Importance](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/fig4.jpg)
*Figure 4: 각 피처 프로파일 범주별 피처 중요도(Feature Importance) 분석 결과*

![Table 4: Contribution of features on the performance of the LightGBM classifier](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/table4.jpg)
*Table 4: 피처 결합에 따른 LightGBM 분류기의 점진적 성능 향상 분석(Ablation Study)*
...
Stage 1과 Stage 2의 각 단계에서, Level 0 Base Classifier들의 결과를 가중 통합하기 위한 최적의 Meta-Classifier 조합을 실험적으로 탐색한 결과, **로지스틱 회귀(Logistic Regression, LR)**가 메타 분류기로서 가장 우수한 성능을 보여 채택되었습니다. 로지스틱 회귀가 메타 분류기로 적합한 이유는 (1) 적은 수의 메타 피처(4개)에서도 효과적으로 학습 가능하여 과적합에 매우 강건하며, (2) 각 베이스 분류기의 예측 가중치를 단순하고 직관적인 선형 결합 방식으로 안정적으로 학습할 수 있기 때문입니다.
...
diSBPred는 과적합을 철저히 배제하기 위해 10-fold 교차 검증과 독립적인 Jackknife 검증을 수행하여 공정한 평가를 받았습니다.

![Table 2: Name and definition of the evaluation metric.](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/table2.jpg)
*Table 2: 모델 평가에 사용된 평가지표 정의 (TP, TN, FP, FN, SN, SP, ACC, BACC, PR, F1-score, MCC)*

### 4.1. Stage 1: 개별 시스테인 결합 상태 예측 성능

개별 시스테인 잔기의 결합 상태(Bonding vs. Free)를 분류하는 Stage 1 성능을 평가하기 위해 다양한 Base Classifier를 단독으로 학습시켰을 때의 결과는 다음과 같습니다.

![Table 5: Performance of cysteine bonding state prediction model for different classifiers.](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/table5.jpg)
*Table 5: Stage 1(개별 시스테인 결합 상태 예측)에서 다양한 Base Classifier들의 성능 비교*

- **Stage 1 (Single Cysteine) 예측 성능**: 
  - 다양한 Base Classifier 중 **LightGBM**이 Balanced Accuracy **81.84%**, F1-score **0.903**으로 가장 우수한 단일 성능을 보였습니다.
  - 최종 Stacking 앙상블 모델은 Balanced Accuracy 기준 **82.29%**를 기록했습니다. 이는 개별 시스테인 잔기가 단백질 내부에서 반응에 참여할 준비가 되어 있는지를 매우 정밀하게 판별함을 의미합니다.

### 4.2. Stage 2: 이황화 결합 연결성 예측 성능

Stage 1의 출력을 거쳐 선별된 시스테인 쌍들의 실제 연결 여부를 예측하는 Stage 2의 단일 Base Classifier 성능 비교는 다음과 같습니다.

![Table 7: Performance of disulfide bond prediction model for different classifiers.](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/table7.jpg)
*Table 7: Stage 2(이황화 결합 연결성 예측)에서 다양한 Base Classifier들의 성능 비교*

- **Stage 2 (Cysteine-Pair) 예측 성능**:
  - Stage 2에서도 트리 기반 앙상블 모델(LightGBM, RDF, ETC, Bagging)이 강력한 분류 성능을 발휘하였으며, 특히 **LightGBM**이 Balanced Accuracy **89.54%**로 우수한 성능을 나타냈습니다.
  - 1단계에서 좁혀진 후보군을 기반으로 공진화 정보와 거리를 결합한 결과, 오답(False Positive)의 비율을 혁신적으로 제어할 수 있었습니다.

### 4.3. Stacking 앙상블 프레임워크 성능 비교

diSBPred는 단순히 단일 모델을 사용하는 것을 넘어, 서로 다른 패밀리의 Base Classifier들을 조합하여 Stacking을 구축했습니다. 다음은 베이스 분류기 구성 조합(SF1, SF2, SF3)에 따른 성능 평가 결과입니다.

![Table 8: Comparison of different stacking framework with a different set of base-classifiers](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/table8.jpg)
*Table 8: 다양한 Base Classifier 조합(SF1, SF2, SF3)에 따른 Stacking 프레임워크의 성능 비교*

- 메타 분류기로 로지스틱 회귀를 채택하여 앙상블 학습을 수행한 결과, 최종 Balanced Accuracy **94.20%**(SF2 프레임워크)의 압도적인 성능을 보였습니다. 

### 4.4. 기존 방법론과의 성능 비교

초보자의 이해를 돕기 위해, 기존의 가장 직관적인 분류 방식이었던 **Nearest Neighbor Algorithm (NNA)**과 두 개의 NNA 기반 모델을 비교한 결과는 다음과 같습니다.

![Table 3: Comparison of the two NNA based models](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/table3.jpg)
*Table 3: Jackknife 교차 검증을 통한 두 가지 NNA 기반 모델(NNA-Model1, NNA-Model2)의 성능 비교*

![Table 9: Comparison of the proposed method diSBPred with the existing NNA method.](/assets/images/2023-08-15-diSBPred-disulfide-bond-prediction/table9.jpg)
*Table 9: 최종 제안 모델 diSBPred와 기존 NNA 모델 간의 성능 비교 (Balanced Accuracy 기준 43.25% 향상)*

- NNA 기반 모델들이 70.62%의 BACC에 그치는 것과 달리, 최종 diSBPred는 **94.20%**의 BACC를 달성하여 성능이 무려 **43.25% 향상**되는 극적인 변화를 보였습니다. 이는 NNA가 단순한 서열 유사성에만 의존하는 반면, diSBPred는 3차원 물리화학적 환경 정보와 공진화 패턴, Stacking 앙상블 결정을 유기적으로 융합하여 분석하기 때문입니다.
...
3. **스태킹 앙상블의 효과적 적용**: 트리 기반(ET, LightGBM), 선형(LR), 거리 기반(KNN) 분류기를 조합한 스태킹 구조와 최종 로지스틱 회귀(LR) 메타 분류기를 통해 개별 모델의 약점을 상호 보완하고, 메타 분류기가 최적의 모델 결합 가중치를 데이터 기반으로 학습합니다.
...
diSBPred는 이황화 결합이라는 단백질 화학의 근본적 문제를 체계적인 피처 엔지니어링과 앙상블 학습으로 접근한 우수한 연구입니다. 비록 딥러닝 시대에 전통적 머신러닝 기반이라는 점에서 향후 발전의 여지가 있으나, 이 연구가 제시한 **2단계 계층적 분류 설계와** **다차원 피처 융합** 전략은 이황화 결합뿐 아니라 다른 번역 후 변형(PTM) 예측 문제에도 범용적으로 적용 가능한 설계 원칙을 제공합니다.

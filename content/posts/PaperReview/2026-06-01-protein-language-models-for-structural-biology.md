...
*   **구조 조건부 서열 생성 (Inverse Folding) 및 ESM-IF1**
    ...
    이를 구현한 대표적 모델인 `ESM-IF1`은 3D 백본의 물리적 공간 기하 정보를 처리하기 위해 **기하학적 벡터 퍼셉트론 그래프 신경망(GVP-GNN)** 구조를 채택했습니다. 
    
    1. **로컬 정규직교 프레임 구축**: 각 아미노산 잔기 $i$의 백본 원자 $N_i, C_{\alpha, i}, C_i$의 3차원 절대 좌표를 사용하여 국소 프레임 $\mathbf{Q}_i = [\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3] \in \text{SO}(3)$를 계산합니다.
       $$\mathbf{v}_1 = \frac{C_i - C_{\alpha, i}}{\|C_i - C_{\alpha, i}\|_2}, \quad \mathbf{v}_2 = \frac{N_i - C_{\alpha, i}}{\|N_i - C_{\alpha, i}\|_2}$$
       $$\mathbf{e}_1 = \frac{\mathbf{v}_1 + \mathbf{v}_2}{\|\mathbf{v}_1 + \mathbf{v}_2\|_2}, \quad \mathbf{e}_2 = \frac{\mathbf{v}_1 - \mathbf{v}_2}{\|\mathbf{v}_1 - \mathbf{v}_2\|_2}, \quad \mathbf{e}_3 = \mathbf{e}_1 \times \mathbf{e}_2$$
    2. **회전 불변 및 동변 특징 추출**:
       * **스칼라 특징(Invariant)**: 잔기 간의 거리 $\|\mathbf{x}_i - \mathbf{x}_j\|_2$, 백본 이면각($\phi_i, \psi_i, \omega_i$)의 사인/코사인 값, 그리고 상대 변위 벡터를 로컬 프레임에 투영한 스칼라 값 $\mathbf{Q}_i^T (\mathbf{x}_j - \mathbf{x}_i)$ 등을 활용합니다.
       * **벡터 특징(Equivariant)**: 잔기 사이의 단위 방향 벡터 $\frac{\mathbf{x}_j - \mathbf{x}_i}{\|\mathbf{x}_j - \mathbf{x}_i\|_2}$ 및 로컬 프레임 축 방향 벡터들을 직접적으로 전파합니다.
    3. **GVP 메시지 패싱**: 
       스칼라 특징과 3D 공간 벡터 특징을 튜플 형태로 동시에 전파하고 상호 게이팅하는 Geometric Vector Perceptron(GVP) 레이어를 거쳐, 수소 결합 네트워크 및 국소 구형/동적 배치를 수학적으로 인지합니다. 이를 통해 타겟 구조에 부합하는 서열 복원(Sequence Recovery) 성능을 극적으로 향상시켰습니다.

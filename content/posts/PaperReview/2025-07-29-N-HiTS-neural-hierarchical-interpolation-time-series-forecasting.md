...
각 블록 $\ell$은 원시 입력 $\mathbf{x}_\ell$을 그대로 학습하지 않고, 스택별로 지정된 풀링 비율(Sampling Rate, $k_\ell$)에 맞추어 **MaxPool을** 먼저 수행합니다.
...
여기서 보간 연산자 $I$는 일반적으로 구현이 단순하고 미분 가능한 **선형 보간(Piecewise Linear Interpolation)** 또는 더 부드러운 곡선을 만드는 **Cubic Spline Interpolation을** 사용합니다. 

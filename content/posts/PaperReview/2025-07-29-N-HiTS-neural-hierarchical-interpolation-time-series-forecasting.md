...
## 💡 2. 연구의 핵심 직관과 아하! 모먼트 (Core Intuition & Aha! Moments)

N-HiTS가 탄생하게 된 배경과 이를 지탱하는 직관적인 유레카 모먼트를 살펴보겠습니다.
...
### 🚀 전문가를 위한 두 가지 "아하! 모먼트 (Aha! Moments)"

#### 💡 Aha! Moment 1: "굳이 미래의 720개 시점을 1대1로 전부 예측해야 할까?"
> **"출력 벡터의 모든 요소를 개별 예측하기보다, 훨씬 적은 수의 핵심 계수(Basis Coefficients, $d$)만 예측하고 이를 타겟 길이($H$)로 부드럽게 연결(Interpolation)하면 어떨까?"**

* **기존의 한계**: 기존의 MLP 기반 예측기(N-BEATS 포함)는 예측 Horizon $H$가 720이면 최종 Linear 레이어가 720차원의 출력을 직접 뱉어냅니다. 이는 파라미터 수가 $O(H)$로 비대해질 뿐 아니라, 각 시점의 출력값이 독립적으로 요동쳐 예측 곡선이 매우 거칠어지고(Volatility) 노이즈에 오버피팅되기 쉽습니다.
* **N-HiTS의 돌파구**: N-HiTS는 MLP 디코더가 오직 $d$개($d \ll H$)의 계수 $\theta$만 뱉어내게 합니다. 그리고 이 $d$개의 값을 **선형 또는 3차 보간법(Linear/Cubic Interpolation)**을 사용해 $H$차원으로 쭉 늘려줍니다. 
* **효과**: 
  1. 출력 레이어의 가중치 파라미터 개수가 $\frac{d}{H}$ 비율로 감소합니다.
  2. 보간법 자체가 일종의 **수학적 규제화(Smoothness Regularization)** 역할을 하여 예측 곡선의 노이즈를 억제하고 안정적인 장기 추세를 포착합니다.

#### 💡 Aha! Moment 2: "입력 데이터를 슥삭 풀링(Pooling)해 버리면, 신경망이 알아서 주파수 대역별로 역할을 분담한다!"
> **"복잡한 주파수 도메인 변환(FFT, Wavelet) 없이, 입력층에 서로 다른 크기의 MaxPool 레이어를 두는 것만으로도 자연스러운 주파수 대역별 신호 분해(Multi-rate sampling)가 이루어진다!"**

* **기존의 한계**: 다중 주파수 분석을 위해 Fourier 변환이나 Wavelet 변환을 직접 신경망에 결합하는 시도가 많았으나, 이는 도메인 지식에 의존적이고 역변환 과정에서 기울기 소실이나 수치적 불안정성이 생길 수 있습니다.
* **N-HiTS의 돌파구**: 입력층 바로 뒤에 단순히 **MaxPool**을 둡니다.
  - **스택 1 (대형 풀링)**: 아주 넓은 커널 크기로 풀링하여 고주파 노이즈를 전부 날려버리고 굵직한 큰 흐름(저주파)만 입력받습니다.
  - **스택 2 (중형 풀링)**: 중간 크기로 풀링하여 중간 대역 주파수를 다룹니다.
  - **스택 3 (풀링 없음)**: 원본 입력 그대로 사용하여 초단기 변동 및 미세 노이즈(고주파)를 다룹니다.
* **효과**: 입력 해상도 자체가 다 다르기 때문에(Multi-rate), 각 스택은 자연스럽게 자신에게 할당된 주파수 밴드에만 전념하게 됩니다.

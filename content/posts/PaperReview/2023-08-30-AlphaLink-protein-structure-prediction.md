...
### 3.2. AlphaFold2 파인튜닝 상세 (Fine-tuning of AlphaFold2)

처음부터 모델을 다시 학습시키는 계산적 비효율성을 피하고자, AlphaLink 연구진은 DeepMind가 공개한 **AlphaFold2 v2.0.0** 가중치를 시작점으로 삼아 네트워크를 파인튜닝(Refinement)했습니다.

* **프레임워크 및 기반 코드**: 파인튜닝 및 구현에는 AlphaFold2의 완전한 PyTorch 재구현체인 **OpenFold v0.1.0** (Jan 2022, commit `894905b`)을 활용했습니다.
* **학습 데이터셋**: **trRosetta** 훈련 세트에서 선별된 약 13,000개의 단백질 구조를 기반으로 삼았으며, 여기에 각 단백질별로 시뮬레이션된 photo-AA 교차결합 데이터를 추가 주입하여 학습을 진행했습니다.
* **MSA 서브샘플링**: 네트워크가 단순히 공진화 정보에만 의존하지 않고 실험적 교차결합 데이터를 주체적으로 활용하도록 강제하기 위해, 학습 단계에서 입력 MSA를 유효 서열 수 $N_{\text{eff}} \in [1, 25]$ 사이에서 무작위(Uniform Distribution)로 서브샘플링하여 주입했습니다.
* **하드웨어 및 자원**: AlphaFold2 논문의 파인튜닝(Refinement Training) 레지임을 준수하되, GPU 메모리 한계로 인해 MSA 클러스터 크기를 확장하지는 않았습니다(도전적 표적을 지향하므로 무방함). **5대의 GPU** 환경에서 **5에포크(Epochs)** 동안 학습을 진행했으며, 약 **5일이** 소요되었습니다.
* **통제 실험(Control Experiment)**: 파인튜닝 과정 자체나 저해상도 MSA 환경 학습이 성능 향상의 주요 원인이 아님을 검증하기 위해, AlphaLink 모델에 교차결합 데이터를 주입하지 않고 예측을 수행하여 원본 **AlphaFold2와** 정량적으로 비교했습니다. 실험 결과 두 모델의 성능은 사실상 동일했습니다:
  - MSA가 완전히 배제된 환경($N_{\text{eff}}=0$): AlphaLink 평균 TM-score 0.322 vs AlphaFold2 평균 TM-score 0.308 (Z-statistic = -0.072)
  - 제한된 MSA 환경($N_{\text{eff}}=10$): AlphaLink 평균 TM-score 0.701 vs AlphaFold2 평균 TM-score 0.702 (Z-statistic = 0.033)
  - 이 결과는 AlphaLink의 정확도 개선이 단순 가중치 미세조정의 효과가 아니라, **주입된 실험적 교차결합 정보를 유기적으로 인코딩하여 활용한 결과**임을 강력히 증명합니다.

---

### 3.3. 평가 설정 (Evaluation Set Up)

AlphaLink의 예측력을 엄밀히 평가하고 원본 AlphaFold2와 공정하게 비교하기 위해 다음과 같은 프로토콜을 정립했습니다.

* **대조군(Baseline)**: DeepMind의 오리지널 **AlphaFold2** 가중치를 탑재한 **OpenFold** 모델을 기본 **대조군으로** 삼았습니다. (OpenFold 개발팀은 원본 AlphaFold2와 동일한 예측 성능을 내는 것을 검증함)
* **템플릿 배제**: 템플릿(Homologous Templates) 구조 정보에 의한 편향을 배제하기 위해, 모든 예측에는 템플릿을 사용하지 않는 **model_5_ptm** 설정을 적용했습니다. 이 모델은 예측된 TM-score(pTM)를 보조 손실 함수(Auxiliary Loss)로 함께 도출합니다.
* **재현성과 비교 가능성 (Determinism)**:
  - 예측의 무작위 변동성을 최소화하고 엄격한 비교를 가능하게 하기 위해 모든 예측 과정을 **결정론적(Deterministic)으로** 통제했습니다.
  - 특히 단기/소형 MSA 표적의 경우, MSA 입력값에 마스킹(Masking out)을 임의 적용하면 실행 시마다 유효 서열 수 **$N_{\text{eff}}$가** 요동쳐 예측 분산(Variance)이 크게 늘어납니다. 따라서 추론 단계에서는 MSA 마스킹을 완전히 **비활성화(Disabled)했습니다**.
  - MSA 크기는 최대 **128개** 서열로 캡핑(Cap)하여 무작위 편차를 통제하였으며, 10개의 독립적인 서브샘플링된 MSA 세트와 10개의 서브샘플링된 교차결합 세트를 고정하여 사용했습니다.
* **기본 표현체**: 별도의 언급이 없는 한 실험 데이터 모델링에는 특정 교차결합제 유형에 맞추어 사전에 정의된 **Soft Label representation을** 기본으로 사용했습니다. (해당 방식이 상대적으로 약간 더 나은 성능을 기록함)
* **주요 평가 지표**: 예측 구조와 네이티브 구조 간의 기하학적 유사도를 정량화하는 **TM-score를** 주 지표로 활용했습니다. (TM-score > 0.5는 동일한 위상/Fold 구조를 가짐을 의미하며, 1.0은 완벽한 일치를 의미함)

---

### 3.4. E. coli 세포 내 대규모 Photo-Leucine 교차결합 실험

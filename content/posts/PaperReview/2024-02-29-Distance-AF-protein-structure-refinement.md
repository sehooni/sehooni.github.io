...
Distance-AF는 AlphaFold2(AF2)의 고정밀 로컬 구조 예측력과 외부 물리적 제약 조건(Distance Constraints)을 결합하여 3차원 구조 모델을 정밀화하는 최적화 프레임워크입니다. 이 모델의 핵심 특징은 새로운 신경망을 학습시키는 것이 아니라, **이미 학습된 AlphaFold2의 가중치(Weights)를 고정한 상태**에서 물리적 제약 조건을 만족하는 최적의 단백질 형태(Conformation)를 탐색한다는 점입니다.

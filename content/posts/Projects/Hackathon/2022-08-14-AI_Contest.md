...
본인의 팀은 이미지처리 분야의 **준지도학습 기반의 항만 구조물 객체 분할 문제에** 참가하였다.
...
크게 **Classification**, **Object Detection**, **Segmentation으로** 나눌 수 있으며,각 기법은 사용 목적에 따라 선택하여 접근하게 된다.
...
위 사진에서 볼 수 있듯이, **좌우 반전**, **상하 반전**, **좌우 이동**, **상하 이동**, **확대** 등의 **위치 변환과**, **BGR2RGB**, **밝기** 등의 **색상 변환**, 그리고 **각도 변환을** 통해 한장 당 88장까지 데이터를 증가시킬 수 있었다.
...
### 결과 정리
안타깝게도 입상에는 실패했다. 첫 AI 대회였고, 팀 단위로 참가하면서 팀원 개인의 역량과 책임감이 중요하게 작용했는데, 마지막에 약간의 미스가 발생했기 때문이다.

그래서일까? 더더욱 아쉬움이 컸다. 마지막엔 결과값이 좋았는데, 이걸 직접적으로 확인하지 못했다는 사실이 특히 그랬다.

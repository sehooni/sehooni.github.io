...
일단 그래프 안에 있는 모든 PRM들을 다 sorting한다. 그러면 위의 그림과 같이 순서대로 나열이 된다. 그 다음에 이제 로테이션 상으로 어떤 그래프에 있는 node u에 대해서 그거에 대응되는 저쪽 나머지 forbidden pair의, u에 대응하는 짝이 되는 친구를 f(u)라고 쓰는 것이다.(이와 같이 쌍을 만드는 것이다.)

...
51:I[24170,["/_next/static/chunks/6b8d09032578b975.js"],"default"]
...
f:["$","p","p-8",{"children":"학습한 모델의 체크포인트는 구글 드라이브에 저장해 두었으므로, code 4-1을 실행하여 코랩 노트북과 자신의 구글 드라이브를 연동한다."}]
...
14:["$","p","p-9",{"children":["각종 인퍼런스 설정을 수행한다. ",["$","code","code-0",{"children":"pretrained_model_name"}],"과 ",["$","code","code-1",{"children":"max_seq_length"}],", ",["$","code","code-2",{"children":"downstream_model_dir"}]," 모두 앞 트레인에서 적용한 그대로 입력하여야 한다."]}]
...
18:["$","h2","h2-1",{"id":"2","className":"text-2xl font-bold mt-8 mb-4","children":"2. 토크나이저 및 모델 불러오기"}]
19:["$","h3","h3-3",{"id":"","className":"text-xl font-bold mt-6 mb-3","children":"토크나이저 로드"}]
1a:["$","p","p-10",{"children":"code 4-3을 실행해 토크나이저를 초기화한다."}]
...
1d:["$","h3","h3-4",{"id":"","className":"text-xl font-bold mt-6 mb-3","children":"체크포인트 로드"}]
1e:["$","p","p-11",{"children":["code 4-4는 ",["$","code","code-0",{"children":"pair_classification_train.ipynb"}],"에서 파인튜닝한 모델의 체크포인트를 읽어 들인다."]}]
...
21:["$","h3","h3-5",{"id":"b","className":"text-xl font-bold mt-6 mb-3","children":"BERT 설정 로드 및 BERT 모델 초기화"}]
22:["$","p","p-12",{"children":["code 4-5는 ",["$","code","code-0",{"children":"pair_classification_train.ipynb"}],"의 파인튜닝 때 사용한 ",["$","code","code-1",{"children":"pretrained_model_name"}],"에 해당하는 모델의 설정값들을 읽어들이며, code 4-6을 실행하면 해당 값대로 BERT 모델을 초기화 한다."]}]
...
27:["$","h3","h3-6",{"id":"","className":"text-xl font-bold mt-6 mb-3","children":"체크포인트 주입하기"}]
28:["$","p","p-13",{"children":["code 4-7은 초기화한 ",["$","strong","strong-0",{"children":"BERT"}],"모델에 code 4-4의 체크포인트를 주입한다"]}]
...
2c:["$","h3","h3-7",{"id":"","className":"text-xl font-bold mt-6 mb-3","children":"평가 모드로 전환"}]
2d:["$","p","p-14",{"children":["이어서 code 4-8을 실행하면 모델이 평가모드로 전환되게 된다. ",["$","strong","strong-0",{"children":"드롭아웃 등 학습 때만 사용하는 기법들을 무효화하는 역할"}],"을 한다."]}]
...
49:T348e,BertForSequenceClassification(
  (bert): BertModel(
...
3b:["$","p","p-19",{"children":["예를 들어 확인된 ",["$","code","code-0",{"children":"authtoken"}],"이 ",["$","code","code-1",{"children":"test123"}],"이라면 다음과 같이 실행 된다."]}]
3c:["$","p","p-20",{"children":"** !mkdir /root/.ngrok2 && echo \"authtoken: test123\" > /root/.ngrok2/ngrok.yml**"}]
3d:["$","h4","h4-10",{"children":"code 4-10"}]
3e:["$","pre","pre-15",{"children":["$","code","code-0",{"className":"hljs language-python","children":["!mkdir /root/.ngrok2 && echo ",["$","span","span-0",{"className":"hljs-string","children":"\"authtoken: (여기 채우세요)\""}]," > /root/.ngrok2/ngrok.yml\n"]}]}]
3f:["$","pre","pre-16",{"children":["$","code","code-0",{"children":"mkdir: cannot create directory ‘/root/.ngrok2’: File exists\n"}]}]
40:["$","h3","h3-10",{"id":"","className":"text-xl font-bold mt-6 mb-3","children":"웹 서비스 시작하기"}]
41:["$","p","p-21",{"children":["code 4-9에서 정의한 인퍼런스 함수 ",["$","code","code-0",{"children":"inference_fn"}],"을 가지고 code 4-11을 실행하면 웹 서비스를 띄울 수 있다. 파이썬의 플라스크를 활용한 앱이다."]}]
...
45:["$","h1","h1-2",{"id":"","className":"text-3xl font-bold mt-8 mb-4","children":"웹사이트 형태는 다음과 같다."}]
...
48:["$","$L52",null,{"content":"$53"}]
4a:["$","span","span-44",{"className":"hljs-string","children":["f\"중립 ",["$","span","span-0",{"className":"hljs-subst","children":"{neutral_prob}"}],"\""]}]

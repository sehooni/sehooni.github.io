...
27:I[3089,["182","static/chunks/app/%5Bslug%5D/page-ad31c54747687caf.js"],"default"]
...
10:["$","pre","pre-8",{"children":["$","code","code-0",{"children":"# 데이터\nx_train = torch.FloatTensor([[1], [2], [3]])\ny_train = torch.FloatTensor([[1], [2], [3]])\n\n# 모델 초기화\nW = torch.zeros(1, requires_grad=True)\nb = torch.zeros(1, requires_grad=True)\n\n# optimizer 설정\noptimizer = optim.SGD([W, b], lr = 0.01)\n\nnb_epochs = 1000\nfor epoch in range(nb_epochs + 1):\n        \n    # H(x) 계산\n    hypothesis = x_train * W + b\n        \n    # cost 계산\n    cost = torch.mean((hypothesis - y_train) ** 2)\n        \n    # cost로 H(x) 개선\n    optimizer.zero_grad()\n    cost.backward()\n    optimizer.step()\n        \n    # 100번마다 로그 출력\n    if epoch % 100 == 0:\n        print('Epoch {:4d}/{} W: {:.3f}, b: {:.3f} Cost: {:.6f}'.format(\n                epoch, nb_epochs, W.item(), b.item(), cost.item()\n        ))\n"}]}]
11:["$","h2","h2-9",{"id":"high-level-implementaion-with","className":"text-2xl font-bold mt-8 mb-4","children":["High-level implementaion with ",["$","code","code-0",{"children":"nn.Module"}]]}]
...
1b:["$","h2","h2-11",{"id":"c","className":"text-2xl font-bold mt-8 mb-4","children":"Cost"}]
...
23:["$","pre","pre-15",{"children":["$","code","code-0",{"children":"# 데이터\nx_train = torch.FloatTensor([[1], [2], [3]])\ny_train = torch.FloatTensor([[1], [2], [3]])\n\n# 모델 초기화\nmodel = LinearRegressionModel()\n\n# optimizer 설정\noptimizer = optim.SGD(model.parameters(), lr=0.01)\n\nnb_epochs = 1000\nfor epoch in range(nb_epochs + 1):\n    \n    # H(x) 계산\n    prediction = model(x_train)\n        \n    # cost 계산\n    cost = F.mse_loss(prediction, y_train)\n        \n    # cost로 H(x) 개선\n    optimizer.zero_grad()\n    cost.backward()\n    optimizer.step()\n        \n    # 100번 마다 로그 출력\n    if epoch % 100 == 0:\n        params = list(model.parameters())\n        W = params[0].item()\n        b = params[1].item()\n        print('Epoch {:4d}/{} W: {:.3f}, b: {:.3f} Cost: {:.6f}'.format(\n            epoch, nb_epochs, W, b, cost.item()\n        ))\n    \n"}]}]
24:["$","p","p-17",{"children":["점점 ",["$","em","em-0",{"children":"H(x)"}]," 의 ",["$","em","em-1",{"children":"W"}]," 와 ",["$","em","em-2",{"children":"b"}]," 를 조정해서 cost가 줄어드는 것을 볼 수 있다."]}]
25:["$","$L27",null,{}]
29:T170f,

...
36:I[24170,["/_next/static/chunks/6b8d09032578b975.js"],"default"]
37:I[55132,["/_next/static/chunks/6b8d09032578b975.js"],"default"]
...
d:["$","code","code-0",{"children":"CNN에서는 이와 유사하게 필터와 bias가 학습을 시킬 매개변수이다. "}]
e:["$","code","code-1",{"children":"다시 말해, 이미지 처리에서 말하는 필터 연산에 해당하는 것이다."}]
f:["$","h2","h2-2",{"id":"s","className":"text-2xl font-bold mt-8 mb-4","children":"stride"}]
10:["$","p","p-13",{"children":"stride는 지정된 간격으로 필터를 순회하는 간격을 의미한다. 아래의 <그림 6>에서는 stride가 1로 설정된 것이며,\nstride가 2로 설정되면 필터는 2칸씩 이동하면서 합성곱을 계산한다."}]
11:["$","h3","h3-4",{"id":"","className":"text-xl font-bold mt-6 mb-3","children":"그림 5. 합성곱 연산 과정 1"}]
12:["$","p","p-14",{"children":["$","img","img-0",{"src":"https://user-images.githubusercontent.com/84653623/165711865-7153cc40-3954-45d9-9db4-31715447b868.png","alt":"합성곱 연산과정 1"}]}]
13:["$","p","p-15",{"children":"위의 그림과 같이 bias는 항상 하나의 값(1*1)으로 존재하며 필터를 적용한 후 모든 원소에 더해지게 된다.\n즉 CNN을 통해 학습이 거듭되며 필터의 원소값과 bias가 매번 갱신되는 것이다."}]
14:["$","h3","h3-5",{"id":"","className":"text-xl font-bold mt-6 mb-3","children":"그림 6. 합성곱 연산 과정 2"}]
15:["$","p","p-16",{"children":["$","img","img-0",{"src":"https://user-images.githubusercontent.com/84653623/165712067-9aa6e8fb-6d79-492a-9664-e866786d09cd.jpg","alt":"합성곱 연산과정 2"}]}]
16:["$","p","p-17",{"children":["2차원의 입력 데이터가 들어오면, 필터의 윈도우가 일정 간격으로 이동해가며 연산이 적용된다.\n이때 이 이동과정을 ",["$","code","code-0",{"children":"Stride"}],"라고 한다.\n이 과정을 모든 장소에서 진행하고, 위의 사진 오른쪽과 같은 합성곱 연산의 출력이 완성된다."]}]
17:["$","h1","h1-3",{"id":"p","className":"text-3xl font-bold mt-8 mb-4","children":"Padding"}]
18:["$","p","p-18",{"children":[["$","strong","strong-0",{"children":"Padding"}],"은 기존 데이터 주변에 값들을 채워넣어 크기를 키우는 것이다.\nConvolution 과정은 특징을 추출하는 것을 목표로 한다."]}]
19:["$","p","p-19",{"children":["그러나, Padding이 없이 진행되면 Convolution 연산 후에는 ",["$","em","em-0",{"children":"'의도치 않게'"}]," 이미지의 크기가 작아지게 된다.\n이는 Edge 성분이 점차 사라지게 되고, 이에 따라 Edge 근처의 특징을 누락할 수 있다.\n따라서 Padding을 통해 Convolution 연산을 하게 되더라도, 그 결과 이미지가 입력 이미지와 그 크기를 동일하게 유지하면서 특징을 추출할 수 있다."]}]
1a:["$","h2","h2-3",{"id":"p","className":"text-2xl font-bold mt-8 mb-4","children":"Padding의 종류"}]
1b:["$","p","p-20",{"children":"데이터 주변에 어떠한 값을 채우느냐에 따라 padding의 종류가 나뉜다."}]
1c:["$","ul","ul-0",{"children":["\n",["$","li","li-0",{"children":"Zero Padding : 최외각을 모두 pixel 0으로 설정"}],"\n",["$","li","li-1",{"children":"Same Padding : 최외각을 모두 이미지 외곽의 pixel 값으로 사용"}],"\n"]}]
1d:["$","h3","h3-6",{"id":"","className":"text-xl font-bold mt-6 mb-3","children":"그림 7. Zero Padding 원리"}]
1e:["$","p","p-21",{"children":["$","img","img-0",{"src":"https://user-images.githubusercontent.com/84653623/165729269-37476345-7114-4ddb-a790-735ece84d5f7.jpg","alt":"padding"}]}]
1f:["$","p","p-22",{"children":"<그림 7>은 zero padding의 원리를 설명한 그림이다."}]
20:["$","h1","h1-4",{"id":"p","className":"text-3xl font-bold mt-8 mb-4","children":"Pooling"}]
21:["$","p","p-23",{"children":["CNN의 구조를 살펴보면 ",["$","code","code-0",{"children":"Conv → Pooling → Conv → Pooling~~"}],"의 순서로 진행된다."]}]
22:["$","p","p-24",{"children":["Convolution 연산 후, FC Layer(Fully-Connected Layer)로 바로 연결될 경우, 입력 이미지를 그대로 가져와 연산을 진행하게 된다.\n이 때문에 FC Layer에서의 연산량이 기하급수적으로 증가하게 된다. ",["$","code","code-0",{"children":"이러한 연산량을 줄이기 위해 Pooling이라는 과정을 추가하여, 연산 후 크기를 '적당히' 줄이고 특정 feature를 강조한다."}],"\n그렇다면 특정 feature를 강조한다는 것은 무슨 의미일까?"]}]
23:["$","p","p-25",{"children":["특정 feature를 강조한다는 것은 ",["$","code","code-0",{"children":"Convolution 이미지의 특징을 추출하고, 그 결과 속에서 특정 특징을 강조한다"}],"는 의미이다."]}]
24:["$","p","p-26",{"children":"Convolution이 행렬(matrix)의 연산이라면, Pooling은 각 Pixel에서 하나의 값을 뽑아내는 것이다."}]
25:["$","h2","h2-4",{"id":"p","className":"text-2xl font-bold mt-8 mb-4","children":"Pooling의 종류"}]
26:["$","p","p-27",{"children":"pooling은 두 가지 종류로 구성된다."}]
27:["$","ul","ul-1",{"children":["\n",["$","li","li-0",{"children":"Max Pooling : 윈도우 창 내에서 각 원소들 중 가장 큰 값을 추출"}],"\n",["$","li","li-1",{"children":"Average Pooling : 윈도우 창 내에서 각 원소들의 평균 값을 추출"}],"\n"]}]
28:["$","h3","h3-7",{"id":"","className":"text-xl font-bold mt-6 mb-3","children":"그림 8. Pooling의 원리"}]
29:["$","p","p-28",{"children":["$","img","img-0",{"src":"https://user-images.githubusercontent.com/84653623/165735542-9f473be7-2f0a-4c3f-9de8-211121dce50f.png","alt":"pooling"}]}]
2a:["$","p","p-29",{"children":"<그림 8>을 통해 Pooling의 원리를 살펴볼 수 있다."}]
2b:["$","h1","h1-5",{"id":"","className":"text-3xl font-bold mt-8 mb-4","children":"요약하자면..."}]
2c:["$","p","p-30",{"children":"CNN(Convolution Neural Network)은 이미지의 공간정보를 유지하면서 인접 이미지와의 특징을 효과적으로 인식하고 강조하는 방식으로 이미지의 특징을 추출하는 부분과 이미지를 분류하는 부분으로 구성된다.\n특징 추출 영역은 필터를 사용하여 공유 파라미터 수를 최소화하면서 이미지의 특징을 찾는 Convolution Layer와 특징을 강화하고 모으는 Pooling Layer로 구성된다."}]
2d:["$","p","p-31",{"children":"CNN은 필터의 크기를 비롯하여 Stride, Padding과 Pooling의 크기로 출력 데이터 크기를 조절하고, 필터의 개수로 출력 데이터의 채널을 결정한다."}]
2e:["$","p","p-32",{"children":["CNN은 같은 레이어 크기의 FCNN과 비교해 볼 때 학습 파라미터의 양이 현저히 적다. 은닉층이 깊어질 수록 그 차이는 더 커지게 된다.\n",["$","code","code-0",{"children":"즉, CNN은 더 작은 학습 파라미터로 더 높은 인식률을 제공"}],"하는 것이다."]}]
2f:["$","p","p-33",{"children":"본 논문에는 특징 추출과 분류로 구성이 되어있으나, 이번 포스팅에서는 특징 추출 부분에 포커스를 맞추어 진행하였다."}]
30:["$","hr","hr-0",{}]
31:["$","p","p-34",{"children":"PS. 추가 문의사항 및 질문은 환영합니다. 그를 통해 저도 더 성장할 수 있을테니까요. 긴 글 읽어주셔서 감사합니다."}]
32:["$","h1","h1-6",{"id":"r","className":"text-3xl font-bold mt-8 mb-4","children":"Reference"}]
